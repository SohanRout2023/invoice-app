...
const Register = () => {
  ...
  const submitHandler = (e) => {
    ...
    .then(newUser =>{
      console.log(newUser)
      // To get the urll OF the storage the following steps we have to do
      const date = new Date().getTime()   // we will be assigning a unique value for storing image
      const storageRef = ref(storage,`${displayName + date}`);
      uploadBytesResumable(storageRef,file)
      .then(res => {
        console.log(res)
        // to get the url the following code weh have to right 
        getDownloadURL(storageRef)  
        .then(downloadedUrl => {
          console.log('File Availlable at',downloadedUrl)
          ...
          localStorage.setItem('photoURL',newUser.user.photoURL)    // this will help to open it on new tab
        ...
        <div className='login-container'>
            ...
            <div className='login-boxes login-right'>
                ...
                <form onSubmit={submitHandler}>
                    {/* In email section we are using the arrow function to get the target value */}
